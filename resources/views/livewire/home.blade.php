<div>
    <!DOCTYPE html>
    <html lang="en">
    
   
    
    <body class="rbt-header-sticky">
    
        <div id="my_switcher" class="my_switcher">
            <ul>
                <li>
                    <a href="javascript: void(0);" data-theme="light" class="setColor light">
                        <img src="/assets/images/about/sun-01.svg" alt="Sun images"><span title="Light Mode"> Light</span>
                    </a>
                </li>
                <li>
                    <a href="javascript: void(0);" data-theme="dark" class="setColor dark">
                        <img src="/assets/images/about/vector.svg" alt="Vector Images"><span title="Dark Mode"> Dark</span>
                    </a>
                </li>
            </ul>
        </div>
        <!-- Start Header Area -->
   
     
    
        <main class="rbt-main-wrapper">
            <!-- Start Banner Area  -->
            <div class="rbt-banner-area rbt-banner-20 position-relative">
                <div class="wrapper">
                    <div class="circle-sd-wrapper">
                        <div class="circle-shadow-1">
                            <img src="/assets/images/shape/shadow-1.png" alt="Bannder Shadow">
                        </div>
                        <div class="circle-shadow-2">
                            <img src="/assets/images/shape/shadow-2.png" alt="Bannder Shadow">
                        </div>
                        <div class="circle-shadow-3">
                            <img src="/assets/images/shape/shadow-3.png" alt="Bannder Shadow">
                        </div>
                    </div>
                    <div class="shapes-wrapper">
                        <div class="curve-line-1">
                            <img src="/assets/images/shape/curve-line-1.png" alt="Banner Line">
                        </div>
                        <div class="curve-line-2">
                            <img src="/assets/images/shape/curve-line-2.png" alt="Banner Line">
                        </div>
                        <div class="curve-line-3">
                            <img src="/assets/images/shape/curve-line-3.png" alt="Banner Line">
                        </div>
                        <div class="curve-line-4">
                            <img src="/assets/images/shape/curve-line-4.png" alt="Banner Line">
                        </div>
                        <div class="shape-hand scene">
                            <span data-depth="2">
                                <img src="/assets/images/shape/hand-cap.png" alt="Banner hand">
                            </span>
                        </div>
                        <div class="dot-bg">
                            <img src="/assets/images/shape/dot-bg.png" alt="Banner Dot">
                        </div>
                        <div class="dot-element scene">
                            <span data-depth="2">
                                <img src="/assets/images/shape/dot-element.png" alt="Banner Dot">
                            </span>
                        </div>
                    </div>
    
                    <div class="container">
                        <div class="row col-wrap flex-column-reverse flex-xl-row">
                            <div class="col-xl-6">
                                <div class="inner position-relative">
                                    <div class="shape-1 scene">
                                        <span data-depth="3">
                                            <img src="/assets/images/shape/translate.png" alt="">
                                        </span>
                                    </div>
                                    <h6 class="subtitle theme-gradient">Web Quản Lý Môn Học</h6>
                                    <div class="title-in-shape d-flex flex-wrap align-items-center gap-2">
                                        <div class="profile-share">
                                            <a href="#" class="avatar" data-tooltip="Nguyễn Văn A" tabindex="0">
                                                <img src="/assets/images/shape/user-1.png" alt="education">
                                            </a>
                                            <a href="#" class="avatar" data-tooltip="Trần Thị B" tabindex="0">
                                                <img src="/assets/images/shape/avatar-in-text-2.png" alt="education">
                                            </a>
                                        </div>
                                        <h1 class="title">Quản Lý</h1>
                                    </div>
                                    <h1 class="title">Môn Học<span>
                                            <img src="/assets/images/shape/translate-in-text.png" alt="">
                                        </span> Trực Tuyến
                                    </h1>
                                    <h1 class="title">Dễ Dàng & Hiệu Quả</h1>
                                    <p class="description mt--20 mb--40 mb_sm--30">Hệ thống giúp quản lý môn học, theo dõi tiến độ, đánh giá sinh viên và hỗ trợ giảng dạy hiệu quả.</p>
                                    <a class="rbt-btn btn-gradient hover-icon-reverse" href="#">
                                        <span class="icon-reverse-wrapper">
                                            <span class="btn-text">Khám Phá Ngay</span>
                                            <span class="btn-icon"><i class="feather-arrow-right"></i></span>
                                            <span class="btn-icon"><i class="feather-arrow-right"></i></span>
                                        </span>
                                    </a>
                                </div>
                            </div>
                            <div class="col-xl-6">
                                <div class="language-world">
                                    <div class="world">
                                        <img src="/assets/images/shape/world.png" alt="">
                                    </div>
                                    <div class="flages">
                                        <div class="flag" data-tooltip="Australia" tabindex="0">
                                            <img src="/assets/images/shape/flag-australia.png" alt="">
                                        </div>
                                        <div class="flag" data-tooltip="Canada" tabindex="0">
                                            <img src="/assets/images/shape/flag-canada.png" alt="">
                                        </div>
                                        <div class="flag" data-tooltip="China" tabindex="0">
                                            <img src="/assets/images/shape/flag-china.png" alt="">
                                        </div>
                                        <div class="flag" data-tooltip="French" tabindex="0">
                                            <img src="/assets/images/shape/flag-french.png" alt="">
                                        </div>
                                        <div class="flag" data-tooltip="Germany" tabindex="0">
                                            <img src="/assets/images/shape/flag-germany.png" alt="">
                                        </div>
                                        <div class="flag" data-tooltip="Japan" tabindex="0">
                                            <img src="/assets/images/shape/flag-japan.png" alt="">
                                        </div>
                                        <div class="flag" data-tooltip="Korea" tabindex="0">
                                            <img src="/assets/images/shape/flag-korea.png" alt="">
                                        </div>
                                        <div class="flag" data-tooltip="Pakistan" tabindex="0">
                                            <img src="/assets/images/shape/flag-pakistan.png" alt="">
                                        </div>
                                        <div class="flag" data-tooltip="Srilonka" tabindex="0">
                                            <img src="/assets/images/shape/flag-srilonka.png" alt="">
                                        </div>
                                        <div class="flag" data-tooltip="United Kingdom" tabindex="0">
                                            <img src="/assets/images/shape/flag-uk.png" alt="">
                                        </div>
                                        <div class="flag" data-tooltip="United State" tabindex="0">
                                            <img src="/assets/images/shape/flag-usa.png" alt="">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    
                </div>
            </div>
            <!-- End Banner Area  -->
    
            <!-- Start Feature Area -->
            <div class="rbt-feature-area rbt-section-gap">
                <div class="container">
                    <div class="row g-5">
            
                        <!-- single item -->
                        <div class="col-xl-4 col-lg-6 col-md-6 col-12">
                            <div class="rbt-feature-card">
                                <div class="d-sm-flex align-items-center gap-4">
                                    <span class="icon bg-color-pink">
                                        <img src="/assets/images/icons/f-icon-01.png" alt="Biểu tượng">
                                    </span>
                                    <h6 class="card-title mt-4 mt-sm-0">Đội Ngũ <br> Giảng Viên Chuyên Nghiệp</h6>
                                </div>
                                <p class="description">Hệ thống được xây dựng bởi đội ngũ giảng viên giàu kinh nghiệm, cam kết mang đến chất lượng giáo dục tốt nhất.</p>
                            </div>
                        </div>
            
                        <!-- single item -->
                        <div class="col-xl-4 col-lg-6 col-md-6 col-12">
                            <div class="rbt-feature-card">
                                <div class="d-sm-flex align-items-center gap-4">
                                    <span class="icon bg-color-primary">
                                        <img src="/assets/images/icons/f-icon-02.png" alt="Biểu tượng">
                                    </span>
                                    <h6 class="card-title mt-4 mt-sm-0">Học Các Môn <br> Trực Tuyến</h6>
                                </div>
                                <p class="description">Cung cấp các khóa học trực tuyến giúp học viên dễ dàng tiếp cận kiến thức mọi lúc, mọi nơi.</p>
                            </div>
                        </div>
            
                        <!-- single item -->
                        <div class="col-xl-4 col-lg-6 col-md-6 col-12">
                            <div class="rbt-feature-card">
                                <div class="d-sm-flex align-items-center gap-4">
                                    <span class="icon bg-color-secondary">
                                        <img src="/assets/images/icons/f-icon-03.png" alt="Biểu tượng">
                                    </span>
                                    <h6 class="card-title mt-4 mt-sm-0">Thư Viện <br> Giáo Trình Trực Tuyến</h6>
                                </div>
                                <p class="description">Kho tài liệu phong phú, hỗ trợ học viên tra cứu và nâng cao kiến thức dễ dàng.</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            
            <!-- End Feature Area -->
    
            <!-- Start Course Area -->
            <div class="rbt-course-area rbt-section-gapBottom">
                <div class="container">
                    <div class="row mb--30">
                        <div class="col-lg-12">
                            <div class="section-title text-center">
                                <h6 class="b2 mb--15"><span class="theme-gradient">Khóa Học Của Chúng Tôi</span></h6>
                                <h2 class="title w-600">Học Tập Không Giới Hạn, <br> <span class="theme-gradient">Cơ Hội Mở Rộng</span></h2>
                            </div>
                        </div>
                    </div>
                    
    
                    <div class="row mb--40">
                        <div class="col-lg-12">
                            <div class="rbt-course-tab-button-wrap">
                             
                            </div>
                        </div>
                    </div>
    
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="row g-5">
                                @foreach($courses as $course)

                                <!-- Start Single Card  -->
                                <div class="col-xl-4 col-lg-4 col-md-6 col-sm-6 col-12" data-sal-delay="150" data-sal="slide-up" data-sal-duration="800">
                                    <div class="rbt-card variation-03 rbt-hover">
                                        <div class="rbt-card-img">
                                            <a class="thumbnail-link" href="{{ route('courses.show', ['slug' => $course->slug]) }}">
                                                <img src="{{asset('storage/' .  $course->image) }}" alt="{{  $course->course_name }}">
                                                <span class="rbt-btn btn-white icon-hover btn-md">
                                                    <span class="btn-text">Xem Môn Học</span>
                                                <span class="btn-icon"><i class="feather-arrow-right"></i></span>
                                                </span>
                                            </a>
                                        </div>
                                        <div class="rbt-card-body " style="
                                        display: flex;
                                        flex-direction: column;
                                        gap: 0rem;
                                    ">
                                            <div class="rbt-card-top" bis_skin_checked="1">
                                                <div class="rbt-review" bis_skin_checked="1">
                                                    <div class="rating" bis_skin_checked="1">
                                                        <i class="fas fa-star"></i>
                                                        <i class="fas fa-star"></i>
                                                        <i class="fas fa-star"></i>
                                                        <i class="fas fa-star"></i>
                                                        <i class="fas fa-star"></i>
                                                    </div>
                                                    <span class="rating-count"> ({{ $course->feedbacks_count }} Đánh giá)</span>
                                                </div>
                                                <div class="rbt-bookmark-btn" bis_skin_checked="1">
                                                    <a class="rbt-round-btn" title="Bookmark" href="#"><i class="feather-bookmark"></i></a>
                                                </div>
                                            </div>
                                            <ul class="rbt-meta">
                                                <li><i class="feather-book"></i>{{ $course->materials()->count() }} Bài học</li>
                                                <li><i class="feather-users"></i> {{ $course->enrollments_count }} Students</li>
                                            </ul>
                                        </div>
                                        <div class="rbt-card-body" style="padding-top: 0 !important;" >
                                            
                                            <h5 class="rbt-card-title"><a href="{{ route('courses.show', ['slug' => $course->slug]) }}">{{  $course->course_name }}</a>
                                            </h5>
                                            <div class="rbt-card-bottom">
                                                <a class="transparent-button" href="{{ route('courses.show', ['slug' => $course->slug]) }}"><i><svg width="17" height="12" xmlns="http://www.w3.org/2000/svg"><g stroke="#27374D" fill="none" fill-rule="evenodd"><path d="M10.614 0l5.629 5.629-5.63 5.629"/><path stroke-linecap="square" d="M.663 5.572h14.594"/></g></svg></i></a>
                                            </div>
                                        </div>
                                        <div class="card-information">
                                            {!! Str::limit(strip_tags($course->short_description), 100, '...') !!}
                                            <hr>
                                        </div>
                                     
                                    </div>
                                </div>
                                <!-- End Single Card  -->
                                @endforeach
                             <style>
                                .course-description {
    display: -webkit-box;
    -webkit-line-clamp: 2; /* Giới hạn 2 dòng */
    -webkit-box-orient: vertical;
    overflow: hidden;
    text-overflow: ellipsis;
}
                             </style>

                            </div>
                        </div>
                    </div>
    
                    <div class="row mt--55 mt_sm--50">
                        <div class="col-lg-12">
                            <div class="load-more-btn text-center">
                                <a class="rbt-btn btn-gradient btn-lg btn-mobile hover-icon-reverse" href="{{ route('courses.index') }}">
                                    <span class="icon-reverse-wrapper">
                                        <span class="btn-text">Xem Tất Cả Khóa Học</span>
                                        <span class="btn-icon"><i class="feather-arrow-right"></i></span>
                                        <span class="btn-icon"><i class="feather-arrow-right"></i></span>
                                    </span>
                                </a>
                                
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <!-- End Course Area -->
    
            <!-- Start Countdown Area -->
            <div class="rbt-countdown-area bg_image bg_image--6 bg_image_fixed rbt-section-gap" data-black-overlay="5">
                <div class="container">
                    <div class="row g-5 align-items-center">
                        <div class="col-lg-6">
                            <div class="inner">
                                <div class="section-title text-start">
                                    <span class="subtitle bg-white-opacity">Tại Sao Chọn Chúng Tôi</span>
                                    <h2 class="title color-white">Hệ Thống Quản Lý Môn Học Trực Tuyến</h2>
                                    <p class="description has-medium-font-size mt--20 mb--0 color-white opacity-7">
                                        Dễ dàng theo dõi, quản lý và tổ chức môn học trực tuyến một cách hiệu quả. Hỗ trợ giảng viên và học viên tối ưu trải nghiệm học tập.
                                    </p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            
            <!-- End Countdown Area -->
    
            <!-- Start Testimonial Area -->
            <div class="rbt-testimonial-area bg-color-white rbt-section-gap">
                <div class="container">
                    <div class="row">
                        <div class="col-lg-12 mb--60 mb_sm--50">
                            <div class="section-title text-center">
                                <h6 class="b2 mb--15"><span class="theme-gradient">Đánh Giá</span></h6>
                                <h2 class="title w-600">Ý Kiến Của <span class="theme-gradient">Sinh Viên</span></h2>
                            </div>
                        </div>
                    </div>
                    <div class="row g-5">
                        <!-- Đánh giá 1 -->
                        <div class="col-lg-4 col-md-6 col-12">
                            <div class="rbt-testimonial-box">
                                <div class="inner">
                                    <div class="clint-info-wrapper">
                                        <div class="thumb">
                                            <img src="/assets/images/testimonial/client-01.png" alt="Ảnh sinh viên">
                                        </div>
                                        <div class="client-info">
                                            <h5 class="title">Martha Maldonado</h5>
                                            <span>Giám đốc Điều hành <i>@ Google</i></span>
                                        </div>
                                    </div>
                                    <div class="description">
                                        <p class="subtitle-3">Sau khi tham gia khóa học, tôi đã có thêm nhiều kiến thức bổ ích và kỹ năng quan trọng.</p>
                                        <div class="rating mt--20">
                                            <a href="#"><i class="fa fa-star"></i></a>
                                            <a href="#"><i class="fa fa-star"></i></a>
                                            <a href="#"><i class="fa fa-star"></i></a>
                                            <a href="#"><i class="fa fa-star"></i></a>
                                            <a href="#"><i class="fa fa-star"></i></a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Đánh giá 2 -->
                        <div class="col-lg-4 col-md-6 col-12">
                            <div class="rbt-testimonial-box">
                                <div class="inner">
                                    <div class="clint-info-wrapper">
                                        <div class="thumb">
                                            <img src="/assets/images/testimonial/client-02.png" alt="Ảnh sinh viên">
                                        </div>
                                        <div class="client-info">
                                            <h5 class="title">Michael D. Lovelady</h5>
                                            <span>CEO <i>@ Google</i></span>
                                        </div>
                                    </div>
                                    <div class="description">
                                        <p class="subtitle-3">Chương trình học rất dễ hiểu và thực tiễn. Tôi có thể áp dụng ngay vào công việc.</p>
                                        <div class="rating mt--20">
                                            <a href="#"><i class="fa fa-star"></i></a>
                                            <a href="#"><i class="fa fa-star"></i></a>
                                            <a href="#"><i class="fa fa-star"></i></a>
                                            <a href="#"><i class="fa fa-star"></i></a>
                                            <a href="#"><i class="fa fa-star"></i></a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Đánh giá 3 -->
                        <div class="col-lg-4 col-md-6 col-12">
                            <div class="rbt-testimonial-box">
                                <div class="inner">
                                    <div class="clint-info-wrapper">
                                        <div class="thumb">
                                            <img src="/assets/images/testimonial/client-03.png" alt="Ảnh sinh viên">
                                        </div>
                                        <div class="client-info">
                                            <h5 class="title">Valerie J. Creasman</h5>
                                            <span>Nhà thiết kế <i>@ Google</i></span>
                                        </div>
                                    </div>
                                    <div class="description">
                                        <p class="subtitle-3">Hệ thống hỗ trợ học tập rất tốt, giảng viên tận tình và nội dung phong phú.</p>
                                        <div class="rating mt--20">
                                            <a href="#"><i class="fa fa-star"></i></a>
                                            <a href="#"><i class="fa fa-star"></i></a>
                                            <a href="#"><i class="fa fa-star"></i></a>
                                            <a href="#"><i class="fa fa-star"></i></a>
                                            <a href="#"><i class="fa fa-star"></i></a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Kết thúc đánh giá -->
                    </div>
                </div>
            </div>
            
            <!-- End Testimonial Area -->
    
            <!-- Start Blog Area -->
      
            <!-- End Blog Area -->
    
            <!-- Start Footer aera -->
          
            <!-- End Footer aera -->
        </main>
    
        <div class="rbt-progress-parent">
            <svg class="rbt-back-circle svg-inner" width="100%" height="100%" viewBox="-1 -1 102 102">
                <path d="M50,1 a49,49 0 0,1 0,98 a49,49 0 0,1 0,-98" />
            </svg>
        </div>
    
     
    </body>
    
    </html>
</div>
